...
sub generate {
    ($libname,$nclasses,$is_core, my $model_source_dir) =@_;
    if (defined $model_source_dir) {
        $src_path='src/'.$model_source_dir.'/Models';
    } else {
        $model_source_dir='GMCF';
    }
...
#	print $classheader;
	...
	my $classheader = "$gen_src_path/$model_source_dir/Models/$libname.h";
...
sub gen_library_config {
	my @lines=();
	my $header =
"--- # SBA Core Library Configuration Generated by $0 on $date
System:
  Version: 3.0
  Library: $libname
...
sub gen_kernel_wrapper {
	...
	print $SG "// Generated by $0 on $date\n";
	while (my $line=<$ST>) {
...
";
  print "GENERATING $gen_src_path/Services.cc\n";
	...
	print $KW "// Generated by $0 on $date\n";
#		print $line;
...
sub generateGMCF {
   (my $models, my $model_source_dir) =@_;
    if (defined $model_source_dir) {
        $src_path='src/'.$model_source_dir.'/Models';
    } else {
        $model_source_dir='GMCF';
   ...
   if (not -e "$gen_src_path/$model_source_dir/Models") {
     mkdir "$gen_src_path/$model_source_dir";
     mkdir "$gen_src_path/$model_source_dir/Models";
   }
    print "GENERATING $gen_src_path/$model_source_dir/Models/GMCF.h\n";	
	open my $GWH,'>',"$gen_src_path/$model_source_dir/Models/GMCF.h";
	print $GWH '// Wrapper for GMCF generated by $0 on $date. Only parameter is $nmodels
#include "SBA/Types.h"
...
for my $i (keys %{$models}) {
 my $model = $models->{$i};
print $GWH "\t\t".'int64_t '.$model.'(SBA::System* sba_sysptr, SBA::Tile* sba_tileptr, uint64_t);'."\n";
}
print $GWH '
};
';	
    close $GWH;
    print "GENERATING $gen_src_path/$model_source_dir/Models/GMCFmodelF.h\n";	
	open my $GWFH,'>',"$gen_src_path/$model_source_dir/Models/GMCFmodelF.h";
...
close $GWFH;
    ...
    print "GENERATING $gen_src_path/$model_source_dir/Models/GMCF.cc\n";	
	open my $GWCC,'>',"$gen_src_path/$model_source_dir/Models/GMCF.cc";
...
for my $i (keys %{$models}) {
 my $model = $models->{$i};
print $GWCC ' 
int64_t GMCF::'.$model.'(SBA::System* sba_sysptr, SBA::Tile* sba_tileptr, uint64_t model_id) {
#ifdef VERBOSE
...
#ifdef VERBOSE
	std::cout << "CHECKING pointers for MODEL '.$model.' ('.$i.'): .tdc file name:" << sba_sysptr->task_description << "; Tile address: "<< sba_tileptr->address << std::endl;
#endif

#ifdef VERBOSE
';
print $GWCC '	std::cout << "\n CASTING System pointer in model '.$model.' ('.$i.')\n";'."\n";
print $GWCC '
#endif
...
print $GWCC '
#ifdef VERBOSE
	std::cout << "LEAVING '.$model.' ('.$i.')" << std::endl;
#endif
