...
--    putStrLn $ show isLocalNewGen 
    if isLocal
        then return $ localpath++"/Gannet/"++libname++".yml" 
        else if isLocalNew
...
readSBAConfigNew_OLD cfgfile =
    let
        ...
        (snm,am,sm,scm) = getSystemYMapsNew yml
    in
        (servicenodeYMapToList snm,newaliasYMapToList am,newserviceYMapToList sm,newinterfaceYMapToList scm)
...
--getSystemYMapsOLD yml = 
--    let 
--        EMap ltry = (n_elem yml)
--        (_,hv)   = head ltry
--        EMap all_entries = (n_elem hv)
--        used_entries = if (length all_entries) <= 5 
--            then tail all_entries -- throw away NService
--            else (reverse (tail (reverse (tail all_entries)))) -- throw away NService and Configurations
--        [serviceYMap, aliasYMap,interfaceYMap,namesYMap] = used_entries              
--        (_,serviceYMap_v)=serviceYMap
--        (_,aliasYMap_v)=aliasYMap
--        (_,interfaceYMap_v)=interfaceYMap
--        (_,namesYMap_v)=namesYMap        
--    --        (_,configsYMap_v)=configsYMap
--    in        
...
getSystemYMapsNew yml =
    ...
    in        
        (servicenodeYMap_v,aliasYMap_v,serviceYMap_v,interfaceYMap_v)        
...
newserviceYMapToList_helper (mk,mv)  =  
    let
            scname=getYStr mk
            scid_yn:swrapper_yn:ctrl_yn:[] = getYSeq mv
